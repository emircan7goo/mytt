...
export default function TrendyolSpecialProducts({ products }: Props) {
  ...
  return (
    <div className="w-full my-3 sm:my-6 max-w-full overflow-hidden">
      <div className="bg-[#0F172A] rounded-2xl sm:rounded-3xl p-3.5 sm:p-8 relative group border border-slate-800 shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-2.5 sm:mb-5">
          ...
          <Link
            href="/"
            className="text-[10px] sm:text-xs font-black text-slate-300 hover:text-orange-400 flex items-center gap-1 transition-colors"
          >
          ...
          {products.slice(0, 10).map((family) => {
            ...
            return (
              <Link
                ...
                href={href}
                className="group/card flex flex-col justify-between shrink-0 w-[135px] sm:w-[220px] bg-slate-950 rounded-xl sm:rounded-2xl border border-slate-800/90 p-2 sm:p-3.5 shadow-xl hover:shadow-2xl hover:border-orange-500 transition-all duration-300 relative overflow-hidden select-none"
              >
                {/* ── Üst Etiketler & Favori Kalp ── */}
                <div className="space-y-1 sm:space-y-2">
                  <div className="flex items-center justify-between gap-1">
                    <span className="px-1 py-0.5 rounded bg-orange-600/20 text-orange-400 border border-orange-500/30 font-black text-[7.5px] sm:text-[9px] uppercase tracking-wider truncate">
                      Kargo Bedava
                    </span>

                    <button
                      onClick={(e) => { e.preventDefault(); }}
                      className="w-5 h-5 sm:w-7 sm:h-7 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-orange-400 transition-colors"
                      title="Favorilere Ekle"
                    >
                      <Heart size={10} className="sm:w-3.5 sm:h-3.5" />
                    </button>
                  </div>

                  {/* ── Ürün Görseli ── */}
                  <div className="w-full aspect-square bg-slate-900/90 rounded-lg sm:rounded-xl p-1.5 sm:p-3 flex items-center justify-center overflow-hidden border border-slate-800/60 group-hover/card:bg-slate-900 transition-colors">
                    {imgSrc ? (
                      <img
                        src={imgSrc}
                  ...
                  <div className="text-left space-y-0.5 pt-0.5">
                    ...
                    <div className="flex items-center gap-1 text-[9px] sm:text-[11px] font-extrabold text-amber-400 pt-0.5">
                      <Star size={9} className="fill-amber-400 text-amber-400 sm:w-3 sm:h-3" />
                      <span>4.9</span>
                    </div>
                  </div>
                </div>
                ...
                {/* ── Fiyatlandırma ── */}
                <div className="pt-1.5 sm:pt-3 mt-1 border-t border-slate-800 text-left">
                  <div className="text-[9px] sm:text-[11px] font-bold text-slate-400 line-through leading-none">
                    {fmt(oldPrice)} TL
                  </div>
                  <div className="text-xs sm:text-base font-black text-orange-400 leading-tight mt-0.5">
