      ...
      >
        ...
        <div className="max-w-[1280px] mx-auto px-4 lg:px-8 h-[64px] flex items-center gap-3">
          ...
          <form onSubmit={handleSearchSubmit} className="hidden md:flex flex-1 max-w-[600px] ml-4">
            <div className="flex items-center w-full h-[46px] rounded-2xl bg-slate-50 border border-slate-200 focus-within:border-emerald-500 focus-within:bg-white focus-within:shadow-[0_4px_20px_rgba(5,150,105,0.06)] transition-all overflow-hidden px-2">
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            >
              ...
              <span className="relative z-10 text-white group-hover:text-emerald-900 mix-blend-difference">Cihazını Sat</span>
            </Link>
                ...
                return (
                  <div key={cat.label} className="h-full flex items-center flex-shrink-0 relative">
                    ...
                    >
                      {cat.label}
                      {cat.megaMenu && (
                        <ChevronDown size={12} className={`transition-transform duration-200 ${isHovered ? 'rotate-180' : ''} ${isActive ? 'text-emerald-600' : 'text-slate-400'}`} />
                      )}
                      {/* Active underline */}
                      {isActive && (
                        <span className="absolute bottom-0 left-3 right-3 h-[2px] bg-emerald-600 rounded-full" />
                      )}
                    </Link>
                  </div>
                );
                ...
                <div className="max-w-[1000px] mx-auto px-6 py-8">
                        ...
                        <div className="flex flex-col space-y-2.5">
                          {group.items.map((item, j) => (
                            <Link key={j} href={item.href} onClick={() => setHoveredCategory(null)}
                              className="text-[14px] font-medium text-slate-700 hover:text-emerald-600 transition-colors w-max">
                              {item.name}
                            </Link>
                          ))}
                        </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end">
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 text-[13px] font-bold text-emerald-600 hover:text-emerald-700 hover:translate-x-1 transition-all"
                    >
