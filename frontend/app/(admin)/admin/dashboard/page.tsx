...
const ORDER_STATUS: Record<string, { label: string; color: string }> = {
  ...
  RELEASED: { label: 'Teslim Edildi',  color: '#F97316' },
      ...
      {totalPending > 0 && (
        <Link href="/admin/approvals" style={{ textDecoration: 'none' }}>
          <div className="flex items-center gap-4 p-4 rounded-2xl border cursor-pointer transition-all hover:brightness-110"
            ...
            <div style={{ width: 42, height: 42, borderRadius: '12px', background: 'rgba(249,115,22,0.15)', border: '1px solid rgba(249,115,22,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <CheckCircle size={20} style={{ color: '#F97316' }} />
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ color: '#F97316', fontWeight: 800, fontSize: '15px', margin: 0 }}>
                {totalPending} kayıt onay bekliyor
              </p>
              <p style={{ color: 'rgba(249,115,22,0.6)', fontSize: '12px', margin: '2px 0 0' }}>
            ...
            <div style={{ padding: '6px 16px', borderRadius: '10px', background: '#F97316', color: '#fff', fontSize: '12px', fontWeight: 800, flexShrink: 0 }}>
              Onayla
            </div>
          </div>
        </Link>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { href: '/admin/builder',          label: 'Site Düzenleyici',   icon: <TrendingUp size={16}/>,  color: '#F97316' },
          ].map((item) => (
            <Link
              key={item.href}
              href={item.href}
