...
export default function AdminSellRequestsPage() {
  ...
  const STAT_CARDS = [
    ...
    { label: 'Kabul', value: stats?.accepted ?? 0, color: '#10b981' },
    { label: 'Tamamlanan', value: stats?.completed ?? 0, color: '#059669' },
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      {bestBid ? (
                        <div>
                          <p style={{ color: '#10b981', fontSize: 14, fontWeight: 800, margin: 0 }}>{fmt(Number(bestBid.amount))}</p>
