...
const SHIP_CFG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  DELIVERED:               { label: 'Teslim Edildi',       color: '#10b981', icon: <PackageCheck size={11}/>    },
};
...
function shippingActions(order: any): { endpoint: string; label: string; color: string; needsInput?: 'trackingCode' | 'notes' }[] {
  ...
  switch (order.shippingStatus) {
    ...
    case 'INSPECTION_PASSED': return [{ endpoint: `/orders/${order.id}/admin-ship`,        label: 'Müşteriye Kargola', color: '#10b981', needsInput: 'trackingCode' }];
    case 'ADMIN_SHIPPED':     return [{ endpoint: `/orders/${order.id}/delivered`,         label: 'Teslim Onayı + Escrow Serbest', color: '#10b981' }];
    default:                  return [];
  }
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: 'relative' }}>
      ...
      {toast && (
        <div style={{
          ...
          color: toast.startsWith('❌') ? '#ef4444' : '#10b981',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '12px' }}>
        {[
          ...
          { label: 'Tamamlandı', value: stats.released,  color: '#10b981' },
              ...
              {loading ? (
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 16px', color: '#10b981', fontSize: '13px', fontWeight: 700, whiteSpace: 'nowrap' }}>{fmt(Number(o.amount))}</td>
                    <td style={{ padding: '14px 16px', whiteSpace: 'nowrap' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div style={{ padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <p style={{ color: '#10b981', fontSize: '15px', fontWeight: 800 }}>{fmt(Number(selected.amount))}</p>
                </div>
                <div style={{ padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    return (
                      <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '5px', opacity: done || active ? 1 : 0.3 }}>
                        <div style={{ width: 16, height: 16, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? 'rgba(16,185,129,0.2)' : active ? `${cfg.color}25` : 'rgba(255,255,255,0.05)', border: done ? '1px solid rgba(16,185,129,0.4)' : active ? `1px solid ${cfg.color}50` : '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }}>
                          {done ? <CheckCircle size={9} color="#10b981"/> : cfg.icon}
                        </div>
                        <span style={{ fontSize: '12px', fontWeight: active ? 700 : 500, color: active ? cfg.color : done ? '#10b981' : 'rgba(248,250,252,0.4)' }}>{cfg.label}</span>
                      </div>
                    );
                  })}
