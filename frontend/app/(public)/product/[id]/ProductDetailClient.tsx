...
export default function ProductDetailClient() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center">
        <div className="w-10 h-10 border-[1px] border-zinc-200 border-t-zinc-900 rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  if (isError || !product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F8FAFC]">
        <h1 className="text-zinc-900 font-thin text-[60px] tracking-tighter mb-4">Yokluk.</h1>
        <p className="text-zinc-500 text-[18px] font-light max-w-sm text-center mb-10">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-zinc-900 selection:bg-zinc-900 selection:text-white pt-[100px] md:pt-[120px] pb-20 overflow-hidden relative">
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 relative z-10">
        <button 
          onClick={() => router.back()}
          className="group flex items-center gap-3 text-zinc-500 hover:text-zinc-900 transition-colors mb-8 md:mb-12"
        >
          <div className="w-10 h-10 rounded-full border border-zinc-200 flex items-center justify-center group-hover:border-zinc-400 transition-colors bg-white shadow-sm">
        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          <div className="w-full lg:w-[50%] flex flex-col gap-8">
            <motion.div 
              ...
              className="relative w-full aspect-[4/5] bg-white rounded-[40px] border border-zinc-100 shadow-[0_20px_80px_rgba(0,0,0,0.03)] flex items-center justify-center overflow-hidden"
            >
              ...
              <div className="absolute top-6 left-6 z-20 flex flex-col gap-2">
                ...
                {product.condition === 'NEW' && (
                  <span className="bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full shadow-md">
                    Sıfır Cihaz
                  </span>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="bg-white p-6 rounded-[24px] border border-zinc-100 shadow-sm flex flex-col gap-2 items-start">
                  <div className="w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mb-2">
                     <ShieldCheck size={20} />
                  </div>
                  <span className="font-bold text-[13px] text-zinc-900">12 Ay Mytt Garantisi</span>
                  <p className="text-[11px] text-zinc-500 leading-relaxed">Cihazınız tüm donanımsal sorunlara karşı 1 yıl koruma altındadır.</p>
               </div>
               <div className="bg-white p-6 rounded-[24px] border border-zinc-100 shadow-sm flex flex-col gap-2 items-start">
                  <div className="w-10 h-10 rounded-full bg-green-50 text-green-600 flex items-center justify-center mb-2">
                     <CheckCircle2 size={20} />
                  </div>
                  <span className="font-bold text-[13px] text-zinc-900">21 Nokta Kontrolü</span>
                  <p className="text-[11px] text-zinc-500 leading-relaxed">Uzman teknisyenlerimiz tarafından A'dan Z'ye test edilmiştir.</p>
               </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-[50%] flex flex-col items-start pt-4 lg:pt-10">
            ...
            >
              ...
              <h1 className="text-[50px] md:text-[64px] font-heading font-thin leading-[1.1] tracking-tight mb-8 text-zinc-900">
                {product.model}
              </h1>

              {/* Hardware Specs Grid (Premium look) */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
                 <div className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-zinc-100 text-center gap-2 shadow-sm">
                    <Smartphone size={20} className="text-zinc-400" strokeWidth={1.5} />
                    <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-500">Kapasite</span>
                    <span className="text-[14px] font-semibold text-zinc-900">{product.storage || '128 GB'}</span>
                 </div>
                 <div className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-zinc-100 text-center gap-2 shadow-sm">
                    <div className="w-5 h-5 rounded-full border border-zinc-200" style={{ backgroundColor: product.color === 'Siyah' ? '#000' : product.color === 'Beyaz' ? '#fff' : product.color === 'Kırmızı' ? '#ef4444' : '#e4e4e7' }} />
                    <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-500">Renk</span>
                    <span className="text-[14px] font-semibold text-zinc-900">{product.color || 'Standart'}</span>
                 </div>
                 <div className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-zinc-100 text-center gap-2 shadow-sm">
                    <Battery size={20} className="text-zinc-400" strokeWidth={1.5} />
                    <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-500">Pil Sağlığı</span>
                    <span className="text-[14px] font-semibold text-zinc-900">
                      {(product.specsJson as any)?.batteryHealth ? `%${(product.specsJson as any).batteryHealth}` : '%85+'}
                    </span>
                 </div>
                 <div className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-zinc-100 text-center gap-2 shadow-sm">
                    <Cpu size={20} className="text-zinc-400" strokeWidth={1.5} />
                    <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-500">İşlemci</span>
                    <span className="text-[14px] font-semibold text-zinc-900">Bionic A Serisi</span>
                 </div>
              </div>

              {/* Price Area */}
              <div className="mb-10 bg-white p-8 rounded-[32px] border border-zinc-100 shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
                {user?.role === 'dealer' ? (
                  <div className="flex flex-col">
                    ...
                    <span className="text-[16px] font-medium text-zinc-400 line-through decoration-zinc-300 mb-1">
                       Perakende: {product.price.toLocaleString('tr-TR')} ₺
                    </span>
                    <span className="text-[48px] font-black tracking-tight flex items-start gap-2 text-zinc-900">
                      {(product.price * (1 - (user.commissionRate || 0.10))).toLocaleString('tr-TR')}
                      <span className="text-[20px] text-zinc-400 mt-3 font-bold">₺</span>
                    </span>
                  </div>
                ) : (
                  <>
                    <span className="block text-[12px] font-bold text-zinc-500 tracking-widest uppercase mb-2">Net Satış Fiyatı</span>
                    <span className="text-[48px] font-light tracking-tight flex items-start gap-2 text-zinc-900 leading-none">
                      {product.price.toLocaleString('tr-TR')} 
                      <span className="text-[20px] text-zinc-400 mt-2 font-bold">₺</span>
                    </span>
                  </>
                )}
              ...
              <div className="pt-6 border-t border-zinc-200 flex flex-col gap-4">
                 <div className="flex items-center gap-4 text-zinc-700 text-[13px] font-medium bg-white p-4 rounded-xl border border-zinc-100 shadow-sm">
                    ...
                    <div>
                       ...
                       <span className="text-zinc-500 text-[11px]">Saat 16:00'a kadar verilen siparişler aynı gün kargoda.</span>
                    </div>
                 </div>
                 <div className="flex items-center gap-4 text-zinc-700 text-[13px] font-medium bg-white p-4 rounded-xl border border-zinc-100 shadow-sm">
                    ...
                    <div>
                       ...
                       <span className="text-zinc-500 text-[11px]">256-bit SSL şifreleme ile güvendesiniz.</span>
                    </div>
                 </div>
              </div>
            </motion.div>
