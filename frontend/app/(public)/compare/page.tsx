...
function getGradeStyle(product: MockProduct): { bg: string; text: string; border: string } {
  switch (product.cosmeticGrade) {
    case 'A+': return { bg: '#ECFDF5', text: '#065F46', border: '#6EE7B7' };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-8 px-4 text-center">
      <div className="relative">
        <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 flex items-center justify-center shadow-lg">
          <BarChart2 size={40} className="text-emerald-600" />
        </div>
    ...
    if (key === 'price') {
      ...
      return (
        <div className="flex flex-col items-center gap-1">
          <span className={`font-black text-lg leading-none ${isLowest ? 'text-emerald-600' : 'text-zinc-800'}`}>
            {product.price.toLocaleString('tr-TR')} ₺
          </span>
          {isLowest && (
            <span className="inline-block text-[10px] font-bold uppercase tracking-wide text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
              En İyi Fiyat
            </span>
          )}
        </div>
      ...
      >
        ...
        {products.map((product) => (
          <div
            ...
            style={{ boxShadow: product.price === minPrice && products.length > 1 ? '0 0 0 2px #059669, 0 8px 32px rgba(5,150,105,0.1)' : undefined }}
          >
            {product.price === minPrice && products.length > 1 && (
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-400 to-emerald-600 rounded-t-3xl" />
            )}
            <button
