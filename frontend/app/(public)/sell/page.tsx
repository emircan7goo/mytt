...
const GRADES = [
  { id: 'A+', label: 'Tertemiz',   desc: 'Sıfır gibi, çizik yok',              color: '#10b981', bg: 'rgba(16,185,129,0.08)',  border: 'rgba(16,185,129,0.25)' },
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white flex items-center justify-center px-4 py-20">
      <div className="max-w-lg w-full">
        <div className="bg-white rounded-3xl border border-slate-200 p-10 shadow-xl shadow-black/5 text-center">
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center mx-auto mb-5 shadow-xl shadow-emerald-500/30">
          ...
          {/* Admin Onayı Durumu */}
          <div className={`rounded-2xl p-5 mb-5 text-left ${adminApproved ? 'bg-emerald-50 border border-emerald-200' : 'bg-amber-50 border border-amber-200'}`}>
            <div className="flex items-center gap-3 mb-2">
              {adminApproved
                ? <><TrendingUp size={18} className="text-emerald-600" /><span className="font-bold text-emerald-800 text-sm">Onaylandı — Bayiler Teklif Verebilir</span></>
                : <><Shield size={18} className="text-amber-600" /><span className="font-bold text-amber-800 text-sm">Admin Onayı Bekleniyor</span></>
              }
            </div>
            <p className={`text-sm leading-relaxed ${adminApproved ? 'text-emerald-700' : 'text-amber-700'}`}>
              {adminApproved
          ...
          >
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              {newBidFlash && <span className="text-emerald-600 font-bold text-sm animate-bounce">+Yeni!</span>}
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pt-24 pb-20">
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="flex items-center gap-1 mb-10 mt-6">
          {STEPS.map((s, i) => (
            <div key={i} className="flex items-center gap-1 flex-1">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold transition-all ${ i < step ? 'bg-zinc-900 text-white' : i === step ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'bg-slate-200 text-slate-400' }`}>
        ...
        {/* Step label */}
        <p className="text-xs uppercase tracking-widest font-bold text-emerald-600 mb-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <button key={key} onClick={() => set(!state)}
                    className={`flex flex-col items-center gap-2 py-4 rounded-2xl border-2 transition-all ${ state ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-zinc-500' }`}>
                    <Icon size={20} />
                    <span className="text-xs font-bold">{label}</span>
                    {state && <Check size={12} className="text-emerald-600" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-bold text-emerald-800">30 Dakikalık Teklif Süreci</span>
              </div>
        ...
        <div className="flex gap-3 mt-8">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button
              ...
              className="flex-1 py-3.5 rounded-xl bg-emerald-600 text-white font-bold text-sm hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed shadow-lg shadow-emerald-600/25">
