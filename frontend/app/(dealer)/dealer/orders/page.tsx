...
const SHIP_STATUS: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  WAREHOUSE_RECEIVED:      { label: 'Merkeze Ulaştı',      color: '#F97316', icon: <Warehouse size={11}/>        },
  ...
  DELIVERED:               { label: 'Teslim Edildi',       color: '#F97316', icon: <CheckCircle size={11}/>      },
};
...
export default function DealerOrdersPage() {
  ...
  return (
    <div className="flex flex-col gap-6 pb-12 relative">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-[9999] px-5 py-3 rounded-2xl bg-orange-500/15 border border-orange-500/35 text-orange-400 text-sm font-semibold backdrop-blur-xl shadow-xl">
          {toast}
        </div>
      )}
      ...
      {orders.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Tamamlandı',    value: stats.released, color: '#F97316', desc: 'escrow serbest' },
          ].map(s => (
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-orange-400 font-black text-[14px]">
                      {fmt(Number(o.amount))}
                    </td>
                    <td className="px-5 py-4">
                              ...
                              {[
                                ...
                                return (
                                  <div key={step.key} className="flex items-center gap-1">
                                    <div className={active ? 'px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all bg-sky-500/20 border border-sky-500/40 text-sky-400' : done ? 'px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all bg-orange-500/15 border border-orange-500/25 text-orange-400' : 'px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all bg-zinc-800/40 border border-zinc-700/30 text-zinc-400'}>
                                      {done ? '✓ ' : ''}{step.label}
                                    </div>
                                    {idx < arr.length - 1 && (
                                      <div className={`w-4 h-px ${done ? 'bg-orange-500/40' : 'bg-zinc-700/40'}`}/>
                                    )}
                                  </div>
                                );
                              })}
