...
const SELL_STATUS_MAP: Record<SellRequestStatus, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  ...
  ACCEPTED:  { label: 'Kabul Edildi',   color: '#F97316', bg: 'rgba(249,115,22,0.12)',  icon: CheckCircle  },
  ...
  RECEIVED:  { label: 'Depoda',         color: '#F97316', bg: 'rgba(249,115,22,0.12)',  icon: CheckCircle  },
  COMPLETED: { label: 'Tamamlandı',     color: '#F97316', bg: 'rgba(249,115,22,0.12)',  icon: CheckCircle  },
  ...
  return (
    <Link href={`/dealer/buy-requests/${req.id}`}
      style={{
        ...
        border: hasBid ? '1px solid rgba(249,115,22,0.3)' : '1px solid rgba(255,255,255,0.07)',
      ...
      onMouseLeave={e => (e.currentTarget.style.border = hasBid ? '1px solid rgba(249,115,22,0.3)' : '1px solid rgba(255,255,255,0.07)')}
    >
      {/* Resim */}
      ...
      <div style={{ padding: '14px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          {req.hasBox     && <span style={{ background: 'rgba(249,115,22,0.1)', color: '#F97316', fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '99px' }}>Kutulu</span>}
          {req.hasInvoice && <span style={{ background: 'rgba(249,115,22,0.1)', color: '#F97316', fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '99px' }}>Faturalı</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
          ...
          {!hasNoBalance && hasBid
            ? <span style={{ color: '#F97316', fontSize: '12px', fontWeight: 800 }}>Teklifim: {fmt(Number(req.myBid!.amount))}</span>
            : !hasNoBalance && isOpen
    ...
    <div style={{
      ...
      <div style={{ padding: '14px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          {item.hasBox     && <span style={{ background: 'rgba(249,115,22,0.1)', color: '#F97316', fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '99px' }}>Kutulu</span>}
          {item.hasInvoice && <span style={{ background: 'rgba(249,115,22,0.1)', color: '#F97316', fontSize: '10px', fontWeight: 700, padding: '2px 7px', borderRadius: '99px' }}>Faturalı</span>}
        </div>
        ...
        {isAuction ? (
          <div style={{ marginBottom: 10 }}>
            ...
            {hasBid && item.myBid && (
              <p style={{ color: '#F97316', fontSize: '12px', fontWeight: 700, margin: '4px 0 0' }}>Teklifim: {fmt(Number(item.myBid.amount))}</p>
            )}
        ...
        {/* Mesaj */}
        {msg && (
          <p style={{ color: msg.includes('alındı') || msg.includes('verildi') ? '#F97316' : '#ef4444', fontSize: '11px', fontWeight: 600, margin: '0 0 8px' }}>{msg}</p>
        )}
...
export default function DealerBuyRequestsPage() {
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
      ...
      {/* Yeni talep badge */}
      {newBadge > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 18px', borderRadius: '12px', marginBottom: 18, background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.3)' }}>
          <Bell size={16} style={{ color: '#F97316' }} />
          <p style={{ color: '#F97316', fontWeight: 800, fontSize: '13px', margin: 0 }}>{newBadge} yeni müşteri talebi!</p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => router.push('/dealer/sell')}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: '10px', background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.25)', color: '#F97316', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        {[
          ...
          { label: 'Müşteri Tekl.',  value: myBidCustCount, color: '#F97316', icon: TrendingUp  },
        ...
        <>
          ...
          {((tab === 'all' && filteredCust.length === 0 && filteredDeal.length === 0) ||
            ...
            <div style={{ textAlign: 'center', padding: '80px 0' }}>
              ...
              {tab === 'dealer' && (
                <button onClick={() => router.push('/dealer/sell')}
                  style={{ marginTop: 16, padding: '10px 22px', borderRadius: '10px', background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.25)', color: '#F97316', fontSize: '13px', fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
