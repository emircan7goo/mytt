...
function LiveBidFeed({ count, isOpen }: { count: number; isOpen: boolean }) {
  ...
  return (
    <div style={{
      background: flashing ? 'rgba(249,115,22,0.08)' : 'rgba(255,255,255,0.03)',
      borderRadius: '16px',
      border: flashing ? '1px solid rgba(249,115,22,0.3)' : '1px solid rgba(255,255,255,0.07)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        ...
        {isOpen && (
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#F97316', boxShadow: '0 0 6px #F97316', animation: 'pulse 1.5s ease-in-out infinite' }} />
            <span style={{ color: '#F97316', fontSize: '10px', fontWeight: 700 }}>CANLI</span>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginBottom: 14 }}>
        <span style={{ color: flashing ? '#F97316' : '#f8fafc', fontSize: '42px', fontWeight: 900, lineHeight: 1, transition: 'color 0.3s' }}>{count}</span>
        <span style={{ color: 'rgba(248,250,252,0.4)', fontSize: '13px', paddingBottom: 6 }}>bayi teklif verdi</span>
        {flashing && <Bell size={16} style={{ color: '#F97316', paddingBottom: 6, animation: 'bounce 0.5s ease-in-out 3' }} />}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          {history.map((entry, i) => (
            <div key={i} style={{
              ...
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: i === 0 ? '#F97316' : 'rgba(248,250,252,0.2)', flexShrink: 0 }} />
              <span style={{ color: i === 0 ? '#F97316' : 'rgba(248,250,252,0.4)', fontSize: '12px', fontWeight: i === 0 ? 700 : 500 }}>{entry}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div style={{ maxWidth: '820px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Kendi teklifim */}
          {hasBid && !editing && (
            <div style={{ background: 'rgba(249,115,22,0.07)', borderRadius: 16, border: '1px solid rgba(249,115,22,0.25)', padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <CheckCircle size={18} style={{ color: '#F97316' }} />
                <p style={{ color: '#F97316', fontWeight: 700, fontSize: '14px', margin: 0 }}>Teklifiniz Gönderildi</p>
              </div>
              ...
              <p style={{ color: '#F97316', fontSize: '32px', fontWeight: 900, margin: '0 0 10px', fontFamily: 'monospace' }}>
